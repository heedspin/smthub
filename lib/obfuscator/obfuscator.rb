...
class Obfuscator
  ...
  def initialize
    @female_names = []
    File.open(File.join(Rails.root, 'lib/obfuscator/female.txt'), 'r') do |input|
      while line = input.gets
        @female_names.push line.titleize.strip
      end
    end
    @male_names = []
    File.open(File.join(Rails.root, 'lib/obfuscator/male.txt'), 'r') do |input|
      while line = input.gets
        @male_names.push line.titleize.strip
      end
    end
    @last_names = []
    File.open(File.join(Rails.root, 'lib/obfuscator/last.txt'), 'r') do |input|
      while line = input.gets
        @last_names.push line.titleize.strip
      end
    end
    @street_names = []
    File.open(File.join(Rails.root, 'lib/obfuscator/streets.txt'), 'r') do |input|
      while line = input.gets
        @street_names.push line.titleize.strip
      end
    end
    @company_names = []
    File.open(File.join(Rails.root, 'lib/obfuscator/company_names.txt'), 'r') do |input|
      while line = input.gets
        @company_names.push line.titleize.strip
      end
    end
    @adjectives = []
    File.open(File.join(Rails.root, 'lib/obfuscator/adjectives.txt'), 'r') do |input|
      while line = input.gets
        @adjectives.push line.titleize.strip
      end
    end
    @nouns = []
    File.open(File.join(Rails.root, 'lib/obfuscator/nouns.txt'), 'r') do |input|
      while line = input.gets
        @nouns.push line.titleize.strip
      end
    end
    @verbs = []
    File.open(File.join(Rails.root, 'lib/obfuscator/verbs.txt'), 'r') do |input|
      while line = input.gets
